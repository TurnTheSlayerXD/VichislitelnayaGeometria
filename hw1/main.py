...
def task_1():
    ...
    b1 = (tr.rotate(fi=np.pi / 3, axis= (tri[0] + tri[1] + tri[2]) / 3) @ tri.T).T
    
    display(np.array([tri, b1]))
    
    c1 = (tr.reflect(axis=np.array([[-1, 0], [0, 1]]))  @ tr.homotetia() @ tri.T).T
    ...
    e1 = (tr.rotate(fi=np.pi, axis=min_side_center) @ tr.homotetia(axis=min_side_center) @ tri.T).T
...
def task_2():
    k = 3
    sqr = np.array([[1, 1, 1], [1, 1 + k, 1], [1 + k, 1 + k, 1], [1 + k, 1, 1]], dtype=np.float64) 
